...
double GPEvaluatorDesktop::evaluateGPBytecodeReference(const std::vector<struct GPBytecode>& program,
                                                       std::vector<Path>& path, AircraftState& aircraftState,
                                                       double contextArg) {
    VectorPathProvider pathProvider(path, aircraftState.getThisPathIndex());
    return evaluateBytecodeReference(program.data(), program.size(), pathProvider, aircraftState, contextArg);
}
